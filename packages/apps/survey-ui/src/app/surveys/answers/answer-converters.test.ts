import {
  combineAnswersFromMultipleLocations,
  dbAggregatedAnswersToAnswers,
} from "./answer-converters";
...
describe("dbAggregatedAnswersToAnswers", () => {
  it("should return a map of question IDs to aggregated answers", () => {
    ...
    expect(q1Answers[0].location).toBe("L1");

    const q2Answers = result.Q2;
    expect(q2Answers.length).toBe(3);
    expect(q2Answers[0].answer).toBe("A1");
    expect(q2Answers[0].answerCount).toBe(1);
    expect(q2Answers[0].location).toBe("L1");
    expect(q2Answers[1].answer).toBe("A2");
    expect(q2Answers[1].answerCount).toBe(1);
    expect(q2Answers[1].location).toBe("L1");
    expect(q2Answers[2].answer).toBe("A2");
    expect(q2Answers[2].answerCount).toBe(1);
    expect(q2Answers[2].location).toBe("L2");
...
describe("combineAnswersFromMultipleLocations", () => {
  it("should combine the same answers from multiple locations", () => {
    const dbAnswers = [
      {
        location: "L1",
        questionId: "Q1",
    ...
    const result = combineAnswersFromMultipleLocations(dbAnswers);
